...
| 07 | **MIYABI（みやび）** | キャスト | ⛔ 写真なし（ユーザー指定で「?」表示。写真が来たら差し替え） |
...
※ ももこ・りんか の本人コメントは仮（差し替え待ち）。NANA・AROMA は本人コメント反映済み（09-20）
※ MIYABI は 2026-09-19 追加。本人コメント反映済み（09-20「京都から来ました。仲良くしてください！」）。写真は引き続き待ち
※ しおんの追加写真要望あり（未対応）
...
| 項目 | 内容 |
|---|---|
| 営業時間 | PM 21:00 — LAST（概ね翌4:00、JSON-LD closes=04:00、GBPと一致） |
| 定休日 | **水曜日** |
...
**毎回**: ①事実の提供（キャスト入退店・料金・イベント日程。Claudeは憶測記載禁止のため）
...
| 🟡 | ももこ・りんか・NANA・AROMA の**本人コメント差し替え**（現状は仮）|
| 🟡 | しおん の追加写真 |
| ✅ | ~~新規キャスト追加対応~~ NANA・AROMA 追加完了（07-24）|
| ✅ | ~~2周年特典の詳細公開~~ 完了（07-24・8/7〜8/10、オリジナルシャンパン¥15,000＋初回セット半額。NEWS/Event JSON-LD/llms.txt反映。イベント終了後に告知の整理が必要）|
| 🟢 | GBP（Google ビジネスプロフィール）完成 |
| ✅ | ~~hreflang 相互参照 完全化~~ 完了（07-19・全5言語ページに6本セット統一）|
